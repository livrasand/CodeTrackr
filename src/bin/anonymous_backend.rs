...
fn create_access_token(user_id: &str, secret: &str) -> Result<String, String> {
    ...
    let signature = format!("{}.{}", header_b64, payload_b64); // Mock signature
    
    Ok(format!("{}.{}.mock", header_b64, payload_b64))
}
...
pub async fn login_with_account_number(
    ...
    let user_row = sqlx::query(
        "SELECT id, username, account_number, plan, created_at FROM users WHERE account_number = $1"
    )
    .bind(account_number)
    .fetch_optional(&state.db_pool)
    .await
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🚀 CodeTrackr Anonymous Login Backend running on http://{}", addr);

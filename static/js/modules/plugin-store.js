    ...
    grid.innerHTML = plugins
      .map((p) => {
        ...
        return (
          ...
          (p.author_username
            ? `<div style="font-size:11px; color:var(--text-muted); margin-bottom:6px;" onclick="event.stopPropagation(); openPublicProfile('${p.author_username}')">by <span style="cursor:pointer; text-decoration:underline; color:var(--text-dark);">@${p.author_username}</span></div>`
            : "") +
...
function showNetworkConsentModal(pluginId, btn, pluginData) {
  ...
  if (!modal) {
    ...
    modal.style.cssText =
      "display:none; position:fixed; inset:0; background:rgba(0,0,0,.7); z-index:1000; align-items:center; justify-content:center;";
    modal.innerHTML = `
...
export async function openPluginDetailModal(pluginId, installedIds) {
  ...
  try {
    ...
    if (ratingCountEl)
      ratingCountEl.textContent = p.rating_count
        ? `${avg.toFixed(1)} (${p.rating_count})`
        : "No ratings yet";

    // Description
    const descEl = document.getElementById("detail-description");
    if (descEl)
      descEl.innerHTML = linkifyDescription(p.description, _allPlugins);
    ...
    if (authorUsername) {
      const nameEl = document.getElementById("detail-name");
      if (nameEl)
        nameEl.innerHTML = `${p.name} &nbsp;·&nbsp; <span style="cursor:pointer; text-decoration:underline;" onclick="openPublicProfile('${authorUsername}')">@${authorUsername}</span>`;
    }
    ...
    if (actionsEl) {
      const isInstalled =
        installedIds instanceof Set
          ? installedIds.has(String(pluginId))
          : false;
      const isOwn =
        isLoggedIn() && getCurrentUser()?.username === authorUsername;
      let btns = "";
      if (isInstalled) {
    ...
    if (reviewSection)
      reviewSection.style.display = isLoggedIn() ? "block" : "none";
    ...
    if (reviewsEl) {
      if (reviews.length === 0) {
        reviewsEl.innerHTML =
          '<div style="font-size:12px; color:var(--text-muted);">No reviews yet.</div>';
      } else {
...
export async function submitScreenshot() {
  ...
  const caption = document
    .getElementById("detail-screenshot-caption")
    ?.value.trim();
  if (!url) return;
  try {
  ...
  if (hasExternalAccess) {
    if (!warningEl) {
      ...
      warningDiv.style.cssText =
        "background:#fff3cd; border:1px solid #ffeaa7; border-radius:4px; padding:8px; margin-bottom:8px; font-size:11px; color:#856404;";
...
async function submitPublishPlugin() {
  ...
  const description =
    document.getElementById("pub-description").value.trim() || null;
  ...
  if (!/^[a-z0-9-]+$/.test(name)) {
    errorEl.textContent =
      "El nombre debe ser kebab-case (solo letras minúsculas, números y guiones).";
    errorEl.style.display = "block";
    return;
  }
  if (!script) {

...
async function injectLanguageTabs() {
  ...
  try {
    ...
    container
      .querySelectorAll(".code-tab[data-dynamic]")
...
export async function loadLeaderboard(tab) {
  ...
  try {
    ...
    if (entries.length === 0) {
      rowsEl.innerHTML = `<tr><td colspan="5" style="text-align:center; padding: 24px; font-size: 12px; color: var(--text-dark);">No data yet. Start coding to appear here.</td></tr>`;
      return;
    }
    ...
    rowsEl.innerHTML = `<tr><td colspan="5" style="text-align:center; padding: 24px; color: var(--text-dark);">Could not load leaderboard. <a href="#extensions" style="color:var(--accent);">Start tracking</a></td></tr>`;
  }
}
